...
- Supports __PROFILE_TOKEN_0047__ style and TaxLot closed/open style without hardcoding.
...
def extract_sql_context_for_expression(sql: str, expr: str, limit: int = 5000) -> str:
    ...
    if from_match:
        ...
        simple_stop_terms = ["when matched", "when not matched", "commit", "end;"]
        stops = [pos for term in simple_stop_terms if (pos := lower_context.find(term)) > 0]
        if stops:
            context = context[:min(stops)]
        # Cap before whitespace-normalization. Some ODI blocks are very large and this function
        # can be called many times while building lineage.
        if len(context) > limit * 3:
...
def select_alias_logic_or_context(sql: str, expr: str, alias_key: str = "", limit: int = 5000) -> str:
    """
    Return mapped SELECT-alias logic when available; otherwise build SQL context.
    This avoids Python's dict.get(default) eager-evaluation cost for large ODI SQL blocks.
    """
    logic_map = get_select_alias_logic_map()
    key = normalize_identifier(alias_key or expr)
    if key in logic_map:
        return logic_map[key]
    return extract_sql_context_for_expression(sql, expr, limit)
        ...
        for col, expr in update_pairs:
            rows.append({
                **base,
                ...
                "xml_logic_full": select_alias_logic_or_context(sql, expr, normalize_identifier(expr), 5000),
            })
            ...
            for expr in select_exprs:
                ...
                rows.append({
                    **base,
                    ...
                    "has_join": "Y" if re.search(r"\bjoin\b|\(\+\)", sql, flags=re.I) else "N",
                    "sql_excerpt": short(sql, 1200),
                    "xml_logic_full": select_alias_logic_or_context(sql, expr, alias, 5000),
                })
...
def extract_table_tokens(text: str) -> List[str]:
    ...
    tokens += re.findall(
        r"\b(CL_VAL|TXN|APA|APA_CASH|APA_SECURITY|ACATS_BROKER|AR_DIM|BKR_AR_DIM|AR_GRP_SUBDIM|CCY|J\$[A-Z0-9_#$]+|ACG_TP_DIM|CCAL_CIRD_PD_MAP|SRC_STM_DIM|__PROFILE_TOKEN_0029___NONBKR_RJTRUST_TGT|__PROFILE_TOKEN_0039__S_NONBKR_TGT)\b",
        text,
    )
    return unique_keep_order(tokens)
...
def build_avy_review_rules_diff(column_diff_rows: List[Dict[str, str]], logic_rows: List[Dict[str, str]], detection: MappingDetection) -> List[Dict[str, str]]:
    """
    Curated __PROFILE_TOKEN_0047__ review-level grouping.
    ...
    if "__PROFILE_TOKEN_0047__" not in target and "__PROFILE_TOKEN_0047__" not in resources:
        return []
    ...
    add(
        q(["__PROFILE_TOKEN_0016__", "__PROFILE_TOKEN_0017__"]),
        "Missing implementation",
        mapping_logic(["__PROFILE_TOKEN_0016__", "__PROFILE_TOKEN_0017__"], "Mapping expects lookup from CCAL_REPL_OWNER.CL_VAL with CL_SCM_ID = 114."),
        odi_logic(["__PROFILE_TOKEN_0016__", "__PROFILE_TOKEN_0017__"], "Columns are not present in the final XML insert/update target list."),
    ...
    add(
        q(["__PROFILE_TOKEN_0019__"]),
        "Missing implementation",
        mapping_logic(["__PROFILE_TOKEN_0019__"], "Mapping expects CL_VAL.CL_VAL_CODE, likely using short-sale exemption lookup logic."),
        odi_logic(["__PROFILE_TOKEN_0019__"], "Column is not present in the final XML insert/update target list."),
        "**Real gap.** Mapping rule exists but XML does not populate the target column.",
        "Add column and lookup logic to ODI final load.",
    )
    add(
        q(["__PROFILE_TOKEN_0020__"]),
        "Missing implementation",
        mapping_logic(["__PROFILE_TOKEN_0020__"], "Mapping expects CL_VAL.CL_VAL_NM, with CL_SCM_ID = 115."),
        odi_logic(["__PROFILE_TOKEN_0020__"], "Column is not present in the final XML insert/update target list."),
        "**Real gap.** Name value is not loaded.",
        "Add column and CL_VAL lookup with the correct schema ID.",
    )

    # 4 __PROFILE_TOKEN_0061__
    sdira_cols = ["__PROFILE_TOKEN_0025__", "__PROFILE_TOKEN_0031__", "__PROFILE_TOKEN_0032__"]
    add(
        q(sdira_cols),
        "Possible wrong lookup / missing parse",
        mapping_logic(sdira_cols, "Mapping says logic applies only for SRC_STM_ID = 60 and requires parsing TXN.__PROFILE_TOKEN_0052__, for example splitting transaction type and year."),
        odi_logic(sdira_cols, "XML lineage shows __PROFILE_TOKEN_0032__ <- TXN.__PROFILE_TOKEN_0052__, while type code/name come from CL_VAL; visible CL_VAL join appears not tied to parsed __PROFILE_TOKEN_0052__."),
        "**High-risk mismatch.** XML may not implement the intended __PROFILE_TOKEN_0061__ parsing rule.",
        "Validate actual XML alias for CL_VAL. If no dedicated __PROFILE_TOKEN_0061__ lookup exists, add parsing logic for __PROFILE_TOKEN_0052__ and correct CL_VAL join.",
    )

    # 5-6 debit card
    add(
        q(["__PROFILE_TOKEN_0027__"]),
        "Missing source-system condition",
        mapping_logic(["__PROFILE_TOKEN_0027__"], "Mapping says debit card fields should apply only when SRC_STM_ID = 60."),
        odi_logic(["__PROFILE_TOKEN_0027__"], "XML lineage shows __PROFILE_TOKEN_0027__ from TXN.ORIG_TD / staged value, with no clear CASE WHEN SRC_STM_ID = 60."),
    ...
    debit_ccy_cols = ["__PROFILE_TOKEN_0014__", "__PROFILE_TOKEN_0015__"]
    add(
        q(debit_ccy_cols),
        "Missing source-system condition",
    ...
    # 7 __PROFILE_TOKEN_0045__
    add(
        q(["__PROFILE_TOKEN_0045__"]),
        "WHERE-vs-CASE structural difference",
        mapping_logic(["__PROFILE_TOKEN_0045__"], "Mapping describes the rule like a filter: SRC_STM_ID = 60, ORIG_SRC_STM_CODE LIKE 'MM%', SRC_CRT_USRNM = '__PROFILE_TOKEN_0049__'."),
        odi_logic(["__PROFILE_TOKEN_0045__"], "XML uses CASE WHEN ... THEN TXN.ORIG_SRC_STM_CODE END."),
    ...
    # 9 __PROFILE_TOKEN_0043__.
    add(
        q(["__PROFILE_TOKEN_0043__"]),
        "Priority logic expanded",
        mapping_logic(["__PROFILE_TOKEN_0043__"], "Mapping says: if broker account source is __PROFILE_TOKEN_0057__, use TXN.AR_ID; else if AP.__PROFILE_TOKEN_0043__ exists, use it; else use linked broker account."),
        odi_logic(["__PROFILE_TOKEN_0043__"], "XML uses APA_CASH.__PROFILE_TOKEN_0043__ first, then APA_SECURITY.__PROFILE_TOKEN_0043__, then fallback."),
    ...
    # 11 LGCY
    add(
        q(["__PROFILE_TOKEN_0004__"]),
        "XML-only exception",
        mapping_logic(["__PROFILE_TOKEN_0004__"], "Mapping describes dimension lookup logic."),
        odi_logic(["__PROFILE_TOKEN_0004__"], "XML adds special handling: when SRC_STM_ID = 3, dimension ID is forced to 0."),
    ...
    add(
        ...
        odi_logic(["BATCH_DT"], "XML uses J$__PROFILE_TOKEN_0047__.BATCH_DT / __PROFILE_TOKEN_0047__ journal staging."),
        "**Structural lineage difference.** This may be correct if the scenario uses __PROFILE_TOKEN_0047__ journal staging, but it differs from mapping.",
    ...
    add(
        "Final target",
        "Target ambiguity",
        "User requested __PROFILE_TOKEN_0001__ / DRD sheet target is __PROFILE_TOKEN_0047__.",
        "XML scenario name/header/resources indicate __PROFILE_TOKEN_0028__ or runtime __PROFILE_TOKEN_0003__ side-table behavior.",
        "**Environment/target risk.** The XML may not represent the production __PROFILE_TOKEN_0047__ load directly.",
        "Check runtime variable SSDS.__PROFILE_TOKEN_0003__ and confirm whether it resolves to __PROFILE_TOKEN_0047__ or __PROFILE_TOKEN_0028__.",
    )

    # 14 Column count
    mapping_count = len([r for r in column_diff_rows if r.get("status") in {"IN_BOTH", "MAPPING_ONLY"}])
    final_missing = [c for c in ["__PROFILE_TOKEN_0016__", "__PROFILE_TOKEN_0017__", "__PROFILE_TOKEN_0019__", "__PROFILE_TOKEN_0020__"] if by_col.get(c, {}).get("status") == "MAPPING_ONLY"]
    add(
...
def normalize_business_logic_text(text: str) -> str:
    ...
    replacements = {
        "__PROFILE_TOKEN_0055___OPN_MSTR.": "",
        "TAXLOT_DTL_OPN.": "",
        "__PROFILE_TOKEN_0039__S_NONBKR_TGT.": "",
        "__PROFILE_TOKEN_0029___NONBKR_RJTRUST_TGT.": "",
        "CLS___PROFILE_TOKEN_0055__S_NONBKR_TGT.": "",
        "ACG_TP_DIM.": "",
        "SRC_STM_DIM.": "",
        "CL_VAL1.": "",
        "CL_VAL2.": "",
        ...
        "WASH_SALE_F": "__PROFILE_TOKEN_0030__",
        "SBC_WASH_SALE_AMT": "__PROFILE_TOKEN_0013__",
...
def likely_equivalent_logic_row(row: Dict[str, str]) -> Tuple[bool, str]:
    ...
    # Example: same CASE constants, but DRD derives from __PROFILE_TOKEN_0055___OPN_MSTR while ODI derives from __PROFILE_TOKEN_0039__S_NONBKR_TGT.
    # That must remain in mismatches/review-required, even if branch outcomes are equivalent.
    mapping_upper = mapping.upper()
    odi_upper = odi.upper()
    source_drift_pairs = [
        ("__PROFILE_TOKEN_0055___OPN_MSTR", "__PROFILE_TOKEN_0039__S_NONBKR_TGT"),
        ("__PROFILE_TOKEN_0055___OPN_MSTR", "__PROFILE_TOKEN_0029___NONBKR_RJTRUST_TGT"),
    ]
    ...
    lookup_requirements = {
        ...
        "OPN_TXN_EV_TP": ["CL_VAL", "__PROFILE_TOKEN_0055___TXN_EV_TP_ID", "CL_VAL_NM", "86"],
        ...
        "__PROFILE_TOKEN_0055___TP": ["CL_VAL", "__PROFILE_TOKEN_0055___TXN_TP_ID", "CL_VAL_NM", "84"],
    }
...
def split_mismatch_and_equivalent_rows(rows: List[Dict[str, str]]) -> Tuple[List[Dict[str, str]], List[Dict[str, str]]]:
    ...
    for row in rows:
        ...
        if is_eq:
            ...
            if "__PROFILE_TOKEN_0055___OPN_MSTR" in mu and "__PROFILE_TOKEN_0039__S_NONBKR_TGT" in ou and "__PROFILE_TOKEN_0055___OPN_MSTR" not in ou:
                ...
                m["Recommended Action"] = "Confirm whether __PROFILE_TOKEN_0055___OPN_MSTR and __PROFILE_TOKEN_0039__S_NONBKR_TGT are semantically equivalent for this attribute. If not, adjust ODI source or update DRD."
            mismatches.append(m)
...
def run(args: argparse.Namespace) -> Path:
    ...
    if active_profile == "auto":
        ...
        if "__PROFILE_TOKEN_0047__" in target_blob:
            active_profile = "avy"
        elif "__PROFILE_TOKEN_0055__" in target_blob or "TAXLOTS" in target_blob:
