...
def main(argv=None) -> int:
    ...
    p = argparse.ArgumentParser(description='Full DRD/ODI/Generated INSERT comparison flow v17.5 (v16.6.5 compare + v6.2.1 config-driven insert)')
    ...
    def publish_step1_report():
        ...
        published_excel = ''
        if args.report_mode in {'excel','both'}:
            shutil.copy2(src, dst)
            published_excel = str(dst)
        api_manifest = ''
        if args.report_mode in {'api','both'}:
            api_dir = final_dir/'api'
            api_cmd = [
                sys.executable, '-B', str(api_script),
                '--api-dir', str(api_dir),
                '--step1-workbook', str(src),
            ...
            run(api_cmd, here)
            api_manifest = str(api_dir/'manifest.json')
        step1_summary = {
            'step': 1,
            'report_type': 'DRD/ODI comparison report',
            'report_mode': args.report_mode,
            'source_workbook': str(src),
            'published_excel': published_excel,
            'api_manifest': api_manifest,
            'exists_excel': bool(published_excel and Path(published_excel).exists()),
            'exists_api': bool(api_manifest and Path(api_manifest).exists()),
        }
        ...
        return dst if published_excel else src
    ...
    api_script = here/'api_report_builder.py'
    ...
    final_workbook = final_dir/('_internal_full_cycle_report.xlsx' if args.report_mode == 'api' else 'final_full_cycle_report.xlsx')
    ...
    # STEP 4 REPORT PUBLICATION: final workbook/API after generated INSERT tab is added.
    ...
    published_step4_excel = ''
    if args.report_mode in {'excel','both'}:
        shutil.copy2(final_workbook, step4_report)
        published_step4_excel = str(step4_report)
    ...
    if args.report_mode in {'api','both'}:
        ...
        api_cmd = [
            sys.executable, '-B', str(api_script),
            '--api-dir', str(api_dir),
            '--step1-workbook', str(compare_out/'independent_reports'/'independent_reports.xlsx'),
            '--step4-workbook', str(final_workbook),
            '--compare-out', str(compare_out),
            '--insert-out', str(insert_out),
            '--final-dir', str(final_dir),
            '--mode', args.report_mode,
    ...
    # In API-only mode keep final_workbook as internal source but do not publish step4 Excel copy.
    step4_summary = {
        'step': 4,
        'report_type': 'DRD/ODI/generated INSERT full-cycle report',
        'report_mode': args.report_mode,
        'source_workbook': str(final_workbook),
        'published_excel': published_step4_excel,
        'api_manifest': api_manifest,
        'exists_excel': bool(published_step4_excel and Path(published_step4_excel).exists()),
        'exists_api': bool(api_manifest and Path(api_manifest).exists()),
    }
    ...
    summary = {
        ...
        'step4_full_cycle_report': str(step4_report) if args.report_mode in {'excel','both'} else '',
        'api_manifest': str(final_dir/'api'/'manifest.json') if args.report_mode in {'api','both'} else '',
        ...
        'report_generation': 'twice: step1 and step4; each can publish excel, api, or both',
    }
