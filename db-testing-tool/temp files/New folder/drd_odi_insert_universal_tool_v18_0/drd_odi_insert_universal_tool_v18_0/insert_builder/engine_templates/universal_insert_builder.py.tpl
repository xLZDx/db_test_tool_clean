...
def avy_override_expression(col: str, row: Dict[str, str]) -> Tuple[str, str, str]:
    ...
    if col == "__PROFILE_TOKEN_0045__":
        return "CASE WHEN TXN.SRC_STM_ID = 60 AND TXN.ORIG_SRC_STM_CODE LIKE 'MM%' AND TXN.SRC_CRT_USRNM = '__PROFILE_TOKEN_0049__' THEN TXN.ORIG_SRC_STM_CODE END", "DRD_AVY_CASE_COMPILED", "Compiled DRD WHERE-like text into attribute-level CASE to preserve TXN grain."
    if col == "BATCH_DT":
        return "J_TXN.BATCH_DT", "DRD_AVY_DIRECT_WITH_JOIN", "DRD says J$TXN joined by TXN_ID; alias normalized to J_TXN."
    if col == "__PROFILE_TOKEN_0043__":
        return "CASE WHEN BKR_AR_DIM.ORIG_SRC_STM_CD = '__PROFILE_TOKEN_0057__' THEN TXN.AR_ID WHEN APA.__PROFILE_TOKEN_0043__ IS NOT NULL THEN APA.__PROFILE_TOKEN_0043__ ELSE AR_GRP_SUBDIM.LINKED___PROFILE_TOKEN_0043__ END", "DRD_AVY_CASE_COMPILED", "Compiled DRD precedence rule."
    if col == "__PROFILE_TOKEN_0021__":
        return "CASE WHEN TXN.SRC_STM_ID = 5 AND APA.AC_ID IS NOT NULL THEN ORIG_AR_AC_SUBDIM.AC_NUM ELSE ORIG_SRC_STM_AR_DIM.__PROFILE_TOKEN_0021__ END", "DRD_AVY_CONDITIONAL_JOIN_COMPILED", "Compiled DRD __PROFILE_TOKEN_0060__/__PROFILE_TOKEN_0058__ branch to AR_AC_SUBDIM else AR_DIM."
    if col == "__PROFILE_TOKEN_0011__":
        return "CASE WHEN TXN.SRC_STM_ID <> 3 THEN __PROFILE_TOKEN_0007__.__PROFILE_TOKEN_0011__ WHEN TXN.SRC_STM_ID = 3 THEN __PROFILE_TOKEN_0008__.CL_VAL_CODE END", "DRD_AVY_CONDITIONAL_LOOKUP_COMPILED", "Compiled DRD INSTBO1 vs non-INSTBO1 lookup branch."
    if col == "__PROFILE_TOKEN_0004__":
        return "__PROFILE_TOKEN_0007__.__PROFILE_TOKEN_0004__", "DRD_AVY_LOOKUP_DIRECT", "DRD dimension lookup."
    if col == "__PROFILE_TOKEN_0012__":
        return "CASE WHEN TXN.SRC_STM_ID <> 3 THEN __PROFILE_TOKEN_0007__.__PROFILE_TOKEN_0012__ WHEN TXN.SRC_STM_ID = 3 THEN __PROFILE_TOKEN_0008__.CL_VAL_NM END", "DRD_AVY_CONDITIONAL_LOOKUP_COMPILED", "Compiled DRD INSTBO1 vs non-INSTBO1 lookup branch."
    if col == "__PROFILE_TOKEN_0027__":
        return "CASE WHEN TXN.SRC_STM_ID = 60 THEN TXN.ORIG_TD END", "DRD_AVY_CASE_COMPILED", "DRD says only ACTDETTD/SRC_STM_ID=60."
    if col == "__PROFILE_TOKEN_0014__":
    ...
    if col == "__PROFILE_TOKEN_0025__":
        return "CASE WHEN TXN.SRC_STM_ID = 60 THEN SUBSTR(TXN.__PROFILE_TOKEN_0052__, 1, 3) END", "DRD_AVY_PARSE_TYPE_CODE_COMPILED", "DRD says parse __PROFILE_TOKEN_0061__ transaction type code directly from TRADE_NUMBER; the same parsed code keys the CL_VAL lookup used for __PROFILE_TOKEN_0031__."
    if col == "__PROFILE_TOKEN_0031__":
        return "CASE WHEN TXN.SRC_STM_ID = 60 THEN __PROFILE_TOKEN_0031___CL_VAL.CL_VAL_NM END", "DRD_AVY_PARSE_CL_VAL_LOOKUP_COMPILED", "DRD source table is CL_VAL; parse TRADE_NUMBER to lookup CL_VAL_CODE, then project CL_VAL_NM."
    if col == "__PROFILE_TOKEN_0032__":
        return "CASE WHEN TXN.SRC_STM_ID = 60 THEN 2000 + TO_NUMBER(SUBSTR(TXN.__PROFILE_TOKEN_0052__, -2)) END", "DRD_AVY_PARSE_COMPILED", "DRD says last two digits + century."
    if col == "__PROFILE_TOKEN_0048__":
    ...
    if col == "__PROFILE_TOKEN_0016__":
        return "__PROFILE_TOKEN_0018__.CL_VAL_CODE", "DRD___PROFILE_TOKEN_0056___VAL_BY_SCM", "DRD source table is CL_VAL; rule constrains CL_SCM_ID=114 and selects CL_VAL_CODE."
    if col == "__PROFILE_TOKEN_0017__":
        return "__PROFILE_TOKEN_0018__.CL_VAL_NM", "DRD___PROFILE_TOKEN_0056___VAL_BY_SCM", "DRD source table is CL_VAL; rule constrains CL_SCM_ID=114 and selects CL_VAL_NM."
    if col == "__PROFILE_TOKEN_0019__":
        return "__PROFILE_TOKEN_0019___CL_VAL.CL_VAL_CODE", "DRD___PROFILE_TOKEN_0056___VAL_BY_SCM", "DRD source table is CL_VAL; rule constrains CL_SCM_ID=114 and selects CL_VAL_CODE."
    if col == "__PROFILE_TOKEN_0020__":
        return "__PROFILE_TOKEN_0020___CL_VAL.CL_VAL_NM", "DRD___PROFILE_TOKEN_0056___VAL_BY_SCM", "DRD source table is CL_VAL; rule constrains CL_SCM_ID=115 and selects CL_VAL_NM."
    return "", "", ""
...
def infer_profile(detection, requested: str) -> str:
    ...
    if "__PROFILE_TOKEN_0047__" in blob:
        return "avy"
    if "__PROFILE_TOKEN_0055__" in blob or "TAXLOTS" in blob:
        return "taxlot"
    return "generic"
...
def comparison_classes(mapping_rows, odi_by_col, profile):
    ...
    if profile == "avy":
        dummy = type("D", (), {"target_table_from_sheet": "__PROFILE_TOKEN_0047__", "target_resources_from_xml": ["__PROFILE_TOKEN_0047__"]})()
...
def normalize_rule_aliases(expr: str, primary_alias: str = "", profile: str = "") -> str:
    ...
    if profile == "taxlot" and primary_alias:
        ...
        for conceptual in ["__PROFILE_TOKEN_0055___OPN_MSTR", "TAXLOT_DTL_OPN", "SUB_LOT_MSTR", "__PROFILE_TOKEN_0055___CLS_MSTR", "TAXLOT_DTL_CLS"]:
...
def sanitize_on_clause(on: str) -> str:
    ...
    out = re.sub(r"\((__PROFILE_TOKEN_0060__|RJ\w+|[^()]*only used[^()]*|[^()]*based dimension[^()]*|[^()]*dimension[^()]*)\)", lambda m: " /* " + sql_block_comment(m.group(1), 300) + " */", out, flags=re.I)
...
def avy_apa_kind(row_or_rule) -> str:
    """Return __PROFILE_TOKEN_0058__/__PROFILE_TOKEN_0059__ when DRD explicitly references ETL Notes APA logic."""
    if isinstance(row_or_rule, dict):
        text = clean(row_or_rule.get("drd_rule", ""))
    else:
        text = clean(row_or_rule)
    u = text.upper()
    # Avoid treating generic prose "First preference is __PROFILE_TOKEN_0053__ and then __PROFILE_TOKEN_0059__" as a
    # single source alias; that row needs its own conditional logic.
    if "__PROFILE_TOKEN_0041__" in u or "__PROFILE_TOKEN_0034__" in u:
        return "__PROFILE_TOKEN_0058__"
    if "__PROFILE_TOKEN_0042__" in u or "__PROFILE_TOKEN_0035__" in u:
        return "__PROFILE_TOKEN_0059__"
    return ""


def avy_alias_prefix(kind: str) -> str:
    return "CASH" if kind == "__PROFILE_TOKEN_0058__" else "SEC"
...
def avy_alias_for_table(kind: str, table: str, target_col: str = "") -> str:
    ...
    if t == "__PROFILE_TOKEN_0040__":
        return f"{prefix}___PROFILE_TOKEN_0040__"
    if t == "__PROFILE_TOKEN_0056__":
        return f"{prefix}___PROFILE_TOKEN_0056__"
    if t in {"__PROFILE_TOKEN_0054__", "__PROFILE_TOKEN_0026__"}:
        return f"{prefix}___PROFILE_TOKEN_0054__"
    if t == "CL_VAL":
        if "DB_CR" in col:
...
def add_avy_etl_note_joins_for_row(row: Dict[str, str]) -> List[Dict[str, str]]:
    """Curated __PROFILE_TOKEN_0058__/__PROFILE_TOKEN_0059__ join group from the DRD ETL Notes contract.
    ...
    code_re = "^__PROFILE_TOKEN_0058__[0-7][0-9]" if kind == "__PROFILE_TOKEN_0058__" else "^__PROFILE_TOKEN_0059__[0-7][0-9]"
    ...
    tac = f"{prefix}___PROFILE_TOKEN_0040__"
    acl = f"{prefix}___PROFILE_TOKEN_0056__"
    nna = f"{prefix}___PROFILE_TOKEN_0054__"
    # Base APA branch from ETL Notes. We keep the type decode join separate and
    # filter on the decoded __PROFILE_TOKEN_0058__/__PROFILE_TOKEN_0059__ code.
    ...
    needs_tac = table in {"__PROFILE_TOKEN_0040__", "__PROFILE_TOKEN_0056__", "__PROFILE_TOKEN_0054__", "__PROFILE_TOKEN_0026__"} or re.search(r"\btxn_avy_cl\b|\btac\.", rule, re.I)
    needs_acl = table == "__PROFILE_TOKEN_0056__" or re.search(r"\bavy_cl\b|\bacl\.", rule, re.I)
    needs_nna = table in {"__PROFILE_TOKEN_0054__", "__PROFILE_TOKEN_0026__"} or re.search(r"net_new\s*_?\s*ast\s*_?\s*cgy|\bnna\.", rule, re.I)
    if needs_tac or needs_acl or needs_nna:
        add("LEFT JOIN", "CCAL_REPL_OWNER.__PROFILE_TOKEN_0040__", tac, f"{tac}.TXN_ID = TXN.TXN_ID AND {tac}.ACTV_F = 'Y' AND {apa}.APA_ID = {tac}.APA_ID")
    if needs_acl:
        add("LEFT JOIN", "CCAL_REPL_OWNER.__PROFILE_TOKEN_0056__", acl, f"{acl}.__PROFILE_TOKEN_0056___ID = {tac}.__PROFILE_TOKEN_0056___ID")
    if needs_nna:
        # DRD source table is __PROFILE_TOKEN_0054__ while prose sometimes says __PROFILE_TOKEN_0026__.
        # Use DRD source contract for the physical table/columns.
        add("LEFT JOIN", "CCAL_REPL_OWNER.__PROFILE_TOKEN_0054__", nna, f"{nna}.__PROFILE_TOKEN_0054___ID = {tac}.__PROFILE_TOKEN_0054___ID")
...
def table_alias_from_rule_token(alias: str) -> str:
    ...
    mapping = {"T": "TXN", "JT": "J_TXN", "J$TXN": "J_TXN", "AP": "APA", "CV": "CL_VAL", "ASD": "AR_GRP_SUBDIM", "SS": "SRC_STM_DIM", "FN": "__PROFILE_TOKEN_0038__", "E": "ENTERPRISE_ENTITY_DIM_V"}
...
def extra_avy_join_rows(target_col: str) -> List[Dict[str, str]]:
    ...
    if col in {"__PROFILE_TOKEN_0043__", "__PROFILE_TOKEN_0044__", "__PROFILE_TOKEN_0036__"}:
    ...
    if col in {"__PROFILE_TOKEN_0014__", "__PROFILE_TOKEN_0023__"}:
        add("LEFT JOIN", "CCAL_REPL_OWNER.APA", "APA", "APA.EXEC_ID = TXN.TXN_ID")
    ...
    if col in {"__PROFILE_TOKEN_0021__", "__PROFILE_TOKEN_0011__", "__PROFILE_TOKEN_0004__", "__PROFILE_TOKEN_0012__"}:
        add("LEFT JOIN", "TRANSACTIONS_OWNER.__PROFILE_TOKEN_0007__", "__PROFILE_TOKEN_0007__", "TXN.LGCY_TRD_CPCTY_TP_ID = __PROFILE_TOKEN_0007__.LGCY_TRD_CPCTY_TP_ID")
    if col in {"__PROFILE_TOKEN_0011__", "__PROFILE_TOKEN_0012__"}:
        add("LEFT JOIN", "CCAL_REPL_OWNER.CL_VAL", "__PROFILE_TOKEN_0008__", "__PROFILE_TOKEN_0008__.CL_VAL_ID = TXN.LGCY_TRD_CPCTY_TP_ID")
    if col in {"__PROFILE_TOKEN_0048__", "__PROFILE_TOKEN_0033__"}:
        add("LEFT JOIN", "CCAL_REPL_OWNER.APA", "APA", "APA.EXEC_ID = TXN.TXN_ID")
        add("LEFT JOIN", "CCAL_REPL_OWNER.FIP", "FIP", "FIP.APA_ID = APA.APA_ID")
    if col in {"__PROFILE_TOKEN_0025__", "__PROFILE_TOKEN_0031__"}:
        add("LEFT JOIN", "CCAL_REPL_OWNER.CL_VAL", "__PROFILE_TOKEN_0031___CL_VAL", "__PROFILE_TOKEN_0031___CL_VAL.CL_VAL_CODE = SUBSTR(TXN.__PROFILE_TOKEN_0052__, 1, 3) AND TXN.SRC_STM_ID = 60", "DRD_AVY_PARSE_CL_VAL_LOOKUP")
    if col in {"__PROFILE_TOKEN_0016__", "__PROFILE_TOKEN_0017__"}:
        add("LEFT JOIN", "CCAL_REPL_OWNER.CL_VAL", "__PROFILE_TOKEN_0018__", "__PROFILE_TOKEN_0018__.CL_SCM_ID = 114", "DRD_CL_VAL_SOURCE_RULE")
    if col == "__PROFILE_TOKEN_0019__":
        add("LEFT JOIN", "CCAL_REPL_OWNER.CL_VAL", "__PROFILE_TOKEN_0019___CL_VAL", "__PROFILE_TOKEN_0019___CL_VAL.CL_SCM_ID = 114", "DRD_CL_VAL_SOURCE_RULE")
    if col == "__PROFILE_TOKEN_0020__":
        add("LEFT JOIN", "CCAL_REPL_OWNER.CL_VAL", "__PROFILE_TOKEN_0020___CL_VAL", "__PROFILE_TOKEN_0020___CL_VAL.CL_SCM_ID = 115", "DRD_CL_VAL_SOURCE_RULE")
    return rows
...
def infer_avy_source_join(row: Dict[str, str], primary_alias: str) -> str:
    ...
    if table == "__PROFILE_TOKEN_0037__":
        return f"{alias}.SRC_TXN_TP = {primary_alias}.SRC_TXN_TP"
    if table == "__PROFILE_TOKEN_0024__":
        return f"{alias}.ACTION_CODE = {primary_alias}.SRC_ACTN_CODE"
    if table == "__PROFILE_TOKEN_0009__":
        return f"{alias}.SRC_TAX_CODE_ID = {primary_alias}.SRC_TAX_CODE_ID AND {alias}.ACTV_F = 'Y'"
    if table == "__PROFILE_TOKEN_0046__":
        return f"{alias}.EMPLOYEE_ID = __PROFILE_TOKEN_0038__1.RESPONSIBLE_PARTY_EMPLOYEE_ID AND {primary_alias}.TD >= {alias}.EFFECTIVE_DATE AND {primary_alias}.TD < {alias}.END_DATE"
    if table == "__PROFILE_TOKEN_0002__":
...
def infer_taxlot_source_join(row: Dict[str, str], primary_alias: str) -> str:
    ...
    if table_u == "SRC_STM_DIM":
        if " AS 6" in rule_u or primary_alias.upper().startswith("__PROFILE_TOKEN_0029__"):
            return "SRC_STM_DIM.SRC_STM_ID = 6"
        return f"{primary_alias}.SRC_STM_ID = SRC_STM_DIM.SRC_STM_ID"
    if table_u == "ACG_TP_DIM":
        src_col = "ACG_TP_CODE" if primary_alias.upper().startswith("__PROFILE_TOKEN_0029__") else "AC_TP_CODE"
...
def infer_cl_val_on(row: Dict[str, str], primary_alias: str) -> str:
    ...
    for c in candidates:
        if c not in skip:
            ...
            break
    fallback = {
        "__PROFILE_TOKEN_0055___TP": "__PROFILE_TOKEN_0055___TXN_TP_ID",
        "OPN_TXN_EV_TP": "__PROFILE_TOKEN_0055___TXN_EV_TP_ID",
    ...
    if m:
        scm = m.group(1)
    scm_fallback = {"__PROFILE_TOKEN_0055___TP": "84", "OPN_TXN_EV_TP": "86", "CLS_TXN_EV_TP": "86", "DRVD_TRD_CPCTY_CD": "104", "DRVD_TRD_CPCTY_NM": "104"}
    ...
    for j in joins:
        ...
        if same_physical and source_alias:
            ...
            for candidate in re.findall(r"\b([A-Za-z][A-Za-z0-9_$#]*)\s*\.", jj.get("on_clause", "")):
                ...
                protected = {primary_alias.upper(), "TXN", "APA", "CASH_APA", "SEC_APA", "J_TXN", "AR_GRP_SUBDIM", "BKR_AR_DIM", "__PROFILE_TOKEN_0038__1", "__PROFILE_TOKEN_0038__2", source_alias.upper()}
...
def build_join_graph(mapping_rows: List[Dict[str, str]], impl_rows: List[Dict[str, str]], profile: str, primary_ref: str, primary_alias: str) -> Tuple[List[Dict[str, str]], List[Dict[str, str]]]:
    ...
    for row in mapping_rows:
        ...
        if profile == "avy" and avy_apa_kind(row):
            # __PROFILE_TOKEN_0058__/__PROFILE_TOKEN_0059__ rows are governed by the ETL Notes branch. Do not add
            # generic APA/__PROFILE_TOKEN_0040__/__PROFILE_TOKEN_0056__/CL_VAL aliases, otherwise cash and
            # security branches collide and the SQL becomes semantically wrong.
            joins.extend(add_avy_etl_note_joins_for_row(row))
            continue
        if profile == "avy" and ident(col) in {"__PROFILE_TOKEN_0021__", "__PROFILE_TOKEN_0011__", "__PROFILE_TOKEN_0004__", "__PROFILE_TOKEN_0012__", "__PROFILE_TOKEN_0048__", "__PROFILE_TOKEN_0033__", "__PROFILE_TOKEN_0025__", "__PROFILE_TOKEN_0031__", "__PROFILE_TOKEN_0016__", "__PROFILE_TOKEN_0017__", "__PROFILE_TOKEN_0019__", "__PROFILE_TOKEN_0020__"}:
...
def validate_sql(sql: str, impl_rows: List[Dict[str, str]], joins: List[Dict[str, str]], unresolved_aliases: List[Dict[str, str]]) -> List[Dict[str, str]]:
    ...
    scan_sql = re.sub(r"--[^\n]*", " ", scan_sql)
    up = scan_sql.upper()
    for bad in ["ODI_FINAL_SOURCE", "FROM ODI_FINAL_SOURCE", " __PROFILE_TOKEN_0006__", "__PROFILE_TOKEN_0005__"]:
        if bad in up:
