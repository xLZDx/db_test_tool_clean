...
v18.0 API-first full-cycle flow.
...
def main(argv=None) -> int:
    ...
    p = argparse.ArgumentParser(description="Full DRD/ODI/Generated INSERT comparison flow v18.0 API-first")
    ...
    p.add_argument("--fail-on-business-status", default="", help="Optional CI gate: comma-separated business statuses that should return non-zero, e.g. REVIEW_REQUIRED,INSERT_BLOCKER")
    ...
    if isinstance(manifest.get("business_status"), dict):
        business_status_value = manifest.get("business_status", {}).get("status", "")
    fail_statuses = {s.strip() for s in args.fail_on_business_status.split(",") if s.strip()}
    ci_gate_failed = bool(business_status_value and business_status_value in fail_statuses)
    ...
    summary = {
        ...
        "returncode_semantics": "0 means artifacts were generated unless --fail-on-business-status requested CI failure. Check business_status to know whether review/blockers remain.",
        "ci_gate_failed": ci_gate_failed,
        "fail_on_business_status": sorted(fail_statuses),
    }
    (out / "full_cycle_run_summary.json").write_text(json.dumps(summary, indent=2, ensure_ascii=False), encoding="utf-8")
    print(json.dumps(summary, indent=2, ensure_ascii=False))
    return 3 if ci_gate_failed else 0
