...
class PDMCacheResolver:
    def __init__(self, config: Dict[str, Any] | None = None):
        ...
        # Fast-lookup indexes built after _load_cache()
        self._exact_index: Dict[tuple, "PDMColumn"] = {}
        self._schema_index: Dict[str, List["PDMColumn"]] = {}
        self._load_cache()
        self._build_indexes()
    ...
    def _extract_columns_from_json(self, payload: Any, source_file: str) -> List[PDMColumn]:
        ...
        def _add(schema: str, table: str, column: str, dtype: str, nullable: str, desc: str) -> None:
            key = (schema, table, column)
            if key not in seen and table and column:
                seen.add(key)
                cols.append(PDMColumn(schema, table, column, dtype, nullable, desc, source_file))

        # --- Nested PDM format: {"schema": "...", "name": "TABLE", "columns": [...]} ---
        # Our schema_kb uses "name" for table/column — the flat walker below never sees these
        # because column dicts lack schema/table context.
        for obj in self._walk_objects(payload):
            ...
            if not isinstance(raw_cols, list):
                continue
            tbl_schema = norm_name(obj.get("schema") or obj.get("schema_name") or obj.get("owner") or "")
            tbl_name = norm_name(obj.get("name") or obj.get("table_name") or obj.get("table") or "")
            if not tbl_name:
                continue
            for col_obj in raw_cols:
                if not isinstance(col_obj, dict):
                    continue
                col_name = norm_name(col_obj.get("name") or col_obj.get("column_name") or col_obj.get("column") or "")
                if not col_name:
                    continue
                dtype = str(col_obj.get("data_type") or col_obj.get("datatype") or col_obj.get("type") or "")
                nullable = str(col_obj.get("nullable") or col_obj.get("is_nullable") or "")
                desc = str(col_obj.get("description") or col_obj.get("comment") or "")
                _add(tbl_schema, tbl_name, col_name, dtype, nullable, desc)

        # --- Flat format: each object carries schema + table + column fields ---
        for obj in self._walk_objects(payload):
            ...
            if table_key and col_key:
                ...
                _add(schema, table, column, dtype, nullable, desc)
    ...
    def _build_indexes(self) -> None:
        """Build exact-match, schema-partitioned, and table-level indexes."""
        self._exact_index = {}
        self._schema_index: Dict[str, List["PDMColumn"]] = {}
        self._table_index: Dict[tuple, List["PDMColumn"]] = {}
        for col in self.columns:
            self._exact_index[(col.schema, col.table, col.column)] = col
            self._schema_index.setdefault(col.schema, []).append(col)
            self._table_index.setdefault((col.schema, col.table), []).append(col)
    ...
    def resolve_row(self, row: Dict[str, Any]) -> PDMResolution:
        ...
        # --- Fast path: exact (schema, table, attribute) hit ---
        if src_schema and src_table and src_attr:
            exact_col = self._exact_index.get((src_schema, src_table, src_attr))
            if exact_col is not None:
                candidate = {
                    "schema": exact_col.schema, "table": exact_col.table,
                    "attribute": exact_col.column, "dtype": exact_col.dtype,
                    "nullable": exact_col.nullable, "description": exact_col.description,
                    "score": 1.0, "reasons": ["schema_exact", "table_exact", "attribute_exact"],
                    "source_file": exact_col.source_file,
                }
                return PDMResolution(
                    src_schema, src_table, src_attr,
                    exact_col.schema, exact_col.table, exact_col.column,
                    1.0, "VALIDATED_EXACT",
                    "DRD source schema/table/attribute validated against PDM cache",
                    [candidate], False, None,
                )

        # --- Early exit: no schema and no table → nothing to match against ---
        if not src_schema and not src_table:
            event = self.backfill.request_backfill("NO_PDM_CANDIDATES", {"drd_row": row})
            return PDMResolution(
                src_schema, src_table, src_attr, "", "", "", 0.0,
                "BACKFILL_REQUESTED", "No source schema or table in DRD row", [], True, event,
            )

        # --- Fuzzy path: narrow candidate pool via table → schema → full fallback ---
        cols_to_score: List[PDMColumn]
        if src_schema and src_table and (src_schema, src_table) in self._table_index:
            cols_to_score = self._table_index[(src_schema, src_table)]
        elif src_schema and src_schema in self._schema_index:
            cols_to_score = self._schema_index[src_schema]
        else:
            cols_to_score = self.columns
...
# Module-level singleton cache: key = (local_kb_dir, schema_kb_glob, hint_index_glob)
# Avoids re-parsing the KB on every request.
_RESOLVER_SINGLETON: Optional["PDMCacheResolver"] = None
_RESOLVER_SINGLETON_KEY: Optional[tuple] = None


def get_resolver(config: Dict[str, Any] | None = None) -> "PDMCacheResolver":
    """Return a cached PDMCacheResolver; re-create only when config changes."""
    global _RESOLVER_SINGLETON, _RESOLVER_SINGLETON_KEY
    cfg = config or {}
    pdm_cfg = cfg.get("pdm_cache", cfg) or {}
    key = (
        pdm_cfg.get("local_kb_dir", "data/local_kb"),
        pdm_cfg.get("schema_kb_glob", "schema_kb_*.json"),
        pdm_cfg.get("hint_index_glob", "hint_index_*.json"),
    )
    if _RESOLVER_SINGLETON is None or _RESOLVER_SINGLETON_KEY != key:
        _RESOLVER_SINGLETON = PDMCacheResolver(config)
        _RESOLVER_SINGLETON_KEY = key
