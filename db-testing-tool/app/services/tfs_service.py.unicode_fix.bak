...
async def fetch_work_item_context(item_id: int, project: str = "") -> dict:
    ...
    if not project:
        raise ValueError("Project must be selected to fetch TFS work item context.")
    url = _api_url(f"wit/workitems/{item_id}?$expand=all&api-version=7.0", project=project)
    headers = {
    ...
    for rel in data.get("relations", []):
        ...
        elif rel_url and "workitems" in rel_url.lower():
            related_id = None
            match = re.search(r"/workitems/(\d+)", rel_url, flags=re.IGNORECASE)
            if match:
    ...
    for link in context.get("hyperlinks", []):
        ...
        if url.startswith("http"):
            ...
            if msal_needed and cert_configured:
                try:
                    from app.services.msal_cert_auth import get_certificate_token
                    token = get_certificate_token()
                    headers_for_link["Authorization"] = f"Bearer {token}"
                except Exception as e:
                    link["content_text"] = f"[Failed to acquire MSAL token: {e}]"
                    continue
