...
def sanitize_drd_insert(
    sql: str, target_cols: set[str], row_limit: int,
    target_fq: Optional[str] = None,
) -> str:
    """Strip SOURCE_MISSING columns from the INSERT and wrap with
    ROWNUM <= N.

    Phase 7.18: when `target_fq` is supplied, rewrite the INSERT target
    schema.table to that value -- the GUI emitter targets the operator-
    configured production table (e.g. TRANSACTIONS_OWNER.AVY_FACT_SIDE)
    but the live test sandbox is a different schema (e.g. IKOROSTELEV).
    Generic across any (schema, table) pair.
    """
    ...
    # Groups: (1) "INSERT [hint] INTO " (2) original-target-fq
    #         (3) " (" (4) col list (5) ") SELECT"
    original_target = m.group(2)
    insert_prefix = m.group(1)
    final_target = target_fq if target_fq else original_target
    cols_block = m.group(4)
    ...
    insert_paren_open = insert_prefix + final_target + m.group(3)
    insert_paren_close = m.group(5)
...
def main() -> int:
    # Phase 7.18 (operator 2026-06-01): generic CLI + env config.  All
    # hardcoded table / schema / credentials replaced by --target-*,
    # --base-*, --user/--password/--dsn/--mode flags (env-var fallbacks).
    # See scripts/_step_config.py for the contract.
    import argparse
    ...
    parser = argparse.ArgumentParser(
        description="Step 2: DRD-driven INSERT, generic across target tables."
    )
    add_common_args(parser)
    cfg = parse_args_to_config(parser)
    print_config_banner(cfg, "Step 2: DRD-driven INSERT")
    ...
    print(f"1) Fetch live target columns ({target_fq})...")
    ...
    if cfg.drd_input_sql_path:
        src = Path(cfg.drd_input_sql_path)
    else:
    ...
    # 3. Sanitize + wrap with ROWNUM + rewrite INSERT target to our sandbox
    print(f"\n3) Sanitize INSERT (strip SOURCE_MISSING + wrap ROWNUM <= {cfg.row_limit} "
          f"+ rewrite target -> {target_fq})...")
    clean_sql = sanitize_drd_insert(drd_insert, target_cols, row_limit=cfg.row_limit, target_fq=target_fq)
    ...
    base_cols = _columns_for(conn, cfg.base_schema, cfg.base_table)
    clean_sql, dropped = strip_bad_joins(
        clean_sql, conn, base_alias=cfg.base_alias, base_cols=base_cols, present_tables=present,
    )
    if dropped:
    ...
    # 4. TRUNCATE
    print(f"\n4) TRUNCATE {target_fq}...")
    ok, rc, rows, err = _ora_exec(
        conn, f"TRUNCATE TABLE {target_fq}", commit=True,
    )
    if not ok:
        print(f"  FAIL: {err}")
        return 1
    ...
    # 6. Snapshot into <target>_DRD (or whatever --snapshot-suffix-drd says)
    # Phase 7.18 fix: previous BEGIN/EXCEPTION PL/SQL block silently failed
    # because `_ora_exec` strips the trailing `;` that the END needs.  Use
    # plain DROP TABLE PURGE (ignore ORA-00942 if table doesn't exist).
    print(f"\n6) Snapshot into {snap_fq_drd}...")
    _ora_exec(conn, f"DROP TABLE {snap_fq_drd} PURGE", commit=True)
    create_sql = f"CREATE TABLE {snap_fq_drd} "
    if cfg.tablespace:
    ...
    ok, _, rows, _ = _ora_exec(
        conn, f"SELECT COUNT(*) FROM {snap_fq_drd}",
        fetch=True,
    )
    cnt = rows[0][0] if rows else "?"
    print(f"\n=== Step 2 DONE: {snap_fq_drd} = {cnt} rows ===")
    conn.close()
    return 0
